...
async function tokenCheck(req,res,next){
    try {
        ...
        // if a user isnt found in our database on the line above, throw a new error to the catch block
        if (!user) {
        throw new Error("User not authorised")
      }
      // if user does exist create a new object in the request object so we can acces further down
      // further down in the line in the request which in this case will be the login controller
      req.authUser = user
